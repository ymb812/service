...
Ответ (только JSON):"""
    ...
    async def generate_profession_detail_question(
        ...
        Возвращает None, если информации уже достаточно
        """
        context_parts = []
        ...
        prompt = f"""Ты задаешь вспомогательные вопросы для себя, чтобы лучше определить профессию "{profession_name}".

        {context_block}

        Это вопрос #{question_number} из максимум 2.

        Приоритет тем для вопросов-пожеланий (сам предлагай варианты ответов):
        1. Интересующий уровень квалификации для профессии
        2. Интересующая специфика профессии
        3. Не задавай сложные вопросы про профессию - я узнаю информацию у тебя

        Учти пункты выше, контекст, задай ОДИН короткий вопрос-пожелание, который еще НЕ был задан и на который нет ответа в контексте.
        Ответь текстом вопроса без кавычек.
        """
