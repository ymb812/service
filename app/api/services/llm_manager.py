...
Ответь ТОЛЬКО текстом вопроса без кавычек."""
    ...
    {{
        ...
        "chat_examples": [
            {{
                "colleague": "Имя коллеги (например: 'Саша (Backend-разработчик)')",
                "request": "Короткое сообщение-запрос (например: 'Эй, а можешь глянуть, почему кнопка не работает на Safari?')",
                "your_response": "Твой ответ (например: 'Сейчас проверю через BrowserStack. Safari опять что-то со своими старыми полифиллами 🤦')",
                "vibe": "Эмоциональный окрас (например: 'Обычная рутина, но приятно помогать')"
            }},
            {{
                "colleague": "...",
                "request": "...",
    ...
    3. **real_cases** — КОНКРЕТНЫЕ задачи с РЕАЛЬНЫМИ сложностями:
       ...
       - Хорошо: "Сделать бесконечный скролл на React, но так, чтобы не лагало на старых iPhone. ✅
    ...
    6. **chat_examples** — РЕАЛЬНЫЕ диалоги из рабочих чатов (Slack, Telegram, Teams):
       ...
       - Покажи разнообразие ситуаций: срочный баг, обычная задача, обсуждение решения, помощь коллеге, обсуждение архитектуры
       - Используй живой язык
       - Примеры:

         **Плохо:**
         - Коллега: "Помоги с задачей"
         - Ты: "Хорошо, помогу" ❌

         **Хорошо:**
         - **Маша (QA):** "Привет! У тебя на странице профиля кнопка 'Сохранить' не кликается на iPhone 12"
         - **Ты:** "Ого, сейчас гляну. А в консоли браузера что-то есть? Можешь скрин кинуть?"
         - **Вайб:** "Обычный баг-репорт, но приятно, что QA тестирует тщательно" ✅
